...
import { Barcode, Box, Clock, Activity, Database } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const fetchStats = async () => {
    try {
      ...
      const todaysScans = data.filter(scan => new Date(scan.scanned_at) >= today).length;
      
      // Calculate unique materials
      const uniqueMaterials = new Set(data.map(scan => scan.material_name)).size;
      
      // Get last scan time
      const lastScanTime = data.length > 0 ? data[0].scanned_at : null;
